...
async def on_message(ws, message):
    if  ("Successfully subscribed!" in message) \
            ...
            or ("QUOTE_COINBASE_SPOT_ETH_USDT" not in message):
        return
    ...
    try:
        ...
        if prediction_base.shape[0] >= sequence_length:
            ...
            if data.get('symbol') == "QUOTE_COINBASE_SPOT_ETH_USDT":
                response_payload = {
...
async def consumer() -> None:
    url = get_ws_token()
    subscriber = '{"action":"subscribe_symbol", "key":"QUOTE_COINBASE_SPOT_ETH_USDT"}'
